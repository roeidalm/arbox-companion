        ...
        async with coordinator._session.request(
            ...
            **({"json": feedback} if feedback is not None else {}),
            ...
            if response.status == 404:
                raise FeedbackError("not_found", "הטופס אינו זמין. ודאו ששרת Arbox Companion מעודכן.")
            if response.status != 200:
                raise FeedbackError("server_error", "שרת Arbox Companion לא הצליח להשלים את הפעולה. נסו שוב.")
            result = await response.json()
            if not isinstance(result, dict):
                raise FeedbackError("server_error", "שרת Arbox Companion החזיר תשובה לא תקינה")
            return result
    except (TimeoutError, aiohttp.ClientError, ValueError) as err:
        raise FeedbackError("unavailable", "HA לא הצליח להגיע לשרת Arbox Companion ברשת הפנימית. נסו שוב.") from err
...
async def _read(hass, token, entry_id=None):
    ...
    if not entries:
        raise FeedbackError("not_configured", "יש להגדיר ולהפעיל את אינטגרציית Arbox Companion ב־HA")
...
async def ws_save(hass, connection, msg):
    ...
    if coordinator is None:
        connection.send_error(msg["id"], "not_configured", "אינטגרציית Arbox Companion אינה זמינה")
        return
    ...
    async with state["lock"]:
        ...
        if not state.get("panel"):
            await panel_custom.async_register_panel(
                ...
                module_url=f"{STATIC_PATH}/panel.js?v=3.5.4",
                sidebar_title=None,
                require_admin=False,
            )
            await panel_custom.async_register_panel(
                ...
                module_url=f"{STATIC_PATH}/app-panel.js?v=3.5.4",
                sidebar_title="Arbox Companion", sidebar_icon="mdi:weight-lifter",
                require_admin=False,
            )
            state["panel"] = True
